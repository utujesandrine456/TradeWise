...
const SalesForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!itemName || isNaN(buyingPrice) || isNaN(sellingPrice) || isNaN(quantity)) {
    ...
    try {
      await createTransaction({
        variables: {
          ...
          description: `Sale of ${itemName} (${itemType})`,
          secondParty: 'Customer',
          financialDetails: {
            amount: sellingPrice * quantity,
            description: `Sale for ${itemName}`,
            type: 'Credit'
          },
        },
      });
    ...
    <div className="font-Urbanist min-h-screen bg-brand-50 flex flex-col animate-in fade-in duration-700">
      ...
      <div className="bg-[#09111E] flex justify-between items-center px-10 py-4 shadow-2xl relative z-20">
        ...
        <div className="flex items-center px-6 py-2.5 rounded-md bg-white/10 backdrop-blur-md border border-white/20 shadow-inner">
          <h3 className="font-bold text-white text-xs">Sales Interface</h3>
        </div>
      </div>
        ...
        <div className="flex-1 py-12 px-6 md:px-12 flex justify-center items-start lg:overflow-y-auto">
          <div className="bg-white rounded-md p-10 w-full max-w-2xl shadow-2xl border border-brand-100 relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-10 relative z-10">
              <div>
                <h2 className="text-2xl font-bold text-[#09111E]">Add Daily Sales</h2>
              ...
              <div className={`text-right text-xl font-black ${draftPL >= 0 ? 'text-emerald-600 bg-emerald-50 border-emerald-100' : 'text-rose-600 bg-rose-50 border-rose-100'} px-6 py-3 rounded-md border shadow-inner`}>
                {isNaN(draftPL) ? '0.00' : draftPL.toLocaleString()} <span className="text-[10px] opacity-40 ml-1">Frw</span>
              </div>
            </div>
            ...
            <form className="grid grid-cols-1 sm:grid-cols-2 gap-8 relative z-10" onSubmit={handleSubmit}>
              ...
              <div className="col-span-1">
                <label className="block text-xs font-bold text-brand-300 mb-3 px-1">Purchase Price (Frw)</label>
                <input
                  type="number"
                  name="buyingPrice"
              ...
              <div className="col-span-1">
                <label className="block text-xs font-bold text-brand-300 mb-3 px-1">Selling Price (Frw)</label>
                <input
                  type="number"
                  name="sellingPrice"
              ...
              <div className="col-span-1">
                <label className="block text-xs font-bold text-brand-300 mb-3 px-1">Category</label>
              ...
              <div className="col-span-1 sm:col-span-2 pt-6">
                ...
                >
                  ...
                  <span className="relative z-10 flex items-center justify-center gap-4">
                    <RotateCcw className="text-xl animate-spin-slow" /> Finalize Transaction
                  </span>
                </button>
              </div>
            </form>
        ...
        <div className="w-full lg:w-[450px] bg-white border-l border-brand-100 p-10 flex flex-col h-full shadow-[inset_20px_0_40px_-20px_rgba(9,17,30,0.05)] relative overflow-hidden">
          ...
          <div className="grid grid-cols-2 gap-6 mb-12 relative z-10">
            ...
            <div className={`rounded-md p-6 border shadow-inner ${totalPL >= 0 ? 'bg-emerald-50 border-emerald-100' : 'bg-rose-50 border-rose-100'}`}>
              ...
              <p className={`text-2xl font-bold  ${totalPL >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                {totalPL >= 0 ? '+' : ''}{totalPL.toLocaleString()} <span className="text-[10px] opacity-40 ml-1">Frw</span>
              </p>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto pr-4 space-y-6 custom-scrollbar relative z-10">
            {trades.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-64 text-brand-200">
                ...
                <p className="text-xs font-medium italic mt-2">Start recording sales to see logs</p>
              </div>
            ) : (
              trades.slice().reverse().map((trade, index) => (
                ...
                >
                  ...
                  <div className="text-right">
                    <p className={`font-bold text-xl  ${trade.profitLoss >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
