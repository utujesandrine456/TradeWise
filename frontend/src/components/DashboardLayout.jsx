...
const DashboardLayout = () => {
  ...
  return (
    <div className="flex h-screen bg-[#fdfcfb] text-black overflow-hidden font-Urbanist selection:bg-brand-100 selection:text-black">
      ...
      {isMobileMenuOpen && (
        <div
          className="fixed inset-0 bg-[#09111E]/60 z-40 lg:hidden backdrop-blur-sm transition-all"
      ...
      `}>
        <div className="p-8 border-b border-brand-50 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-black text-[#09111E] uppercase tracking-tight">Stocka</h1>
          </div>
          <button
            onClick={() => setIsMobileMenuOpen(false)}
            className="lg:hidden text-[#09111E] hover:text-[#09111E] p-2 bg-brand-50 rounded-md border border-brand-100 transition-colors"
          >
            <MdClose size={20} />
          </button>
        </div>
                ...
                <li key={tab.id} className="mt-2">
                  <button
                    ...
                    className={`w-full flex items-center py-4 px-6 rounded-md transition-all duration-300 group ${activeTab === tab.id
                      ? 'bg-[#09111E] text-white shadow-xl shadow-brand-100 scale-[1.02]'
                      : 'text-brand-500 hover:text-[#09111E] hover:bg-brand-50'
                      }`}
                  >
                    <span className={`mr-4 text-xl ${activeTab === tab.id ? 'text-white' : 'group-hover:text-[#09111E] transition-colors'}`}>
                      {tab.icon}
                    </span>
        ...
        <div className="p-8 border-t border-brand-50">
          <button
            ...
            disabled={isLoading}
            className="w-full flex items-center py-5 px-6 text-[#09111E] bg-brand-50 hover:bg-brand-100 rounded-md transition-all duration-300 font-black uppercase tracking-widest text-xs border border-brand-200 active:scale-95 disabled:opacity-50"
          >
            <MdLogout className="mr-4 text-xl text-[#09111E]" />
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-8 p-10 bg-white border-b border-brand-50 shadow-sm relative overflow-hidden">
          <div className="absolute top-0 left-0 w-full h-1 bg-[#09111E] opacity-50" />
          <div className="flex items-center gap-6 relative z-10">
            <button
              onClick={() => setIsMobileMenuOpen(true)}
              className="lg:hidden text-[#09111E] hover:text-[#09111E] p-3 bg-brand-50 rounded-md border border-brand-100 transition-all font-bold"
            >
              <MdMenu size={24} />
            </button>
            <div>
              <h2 className="text-3xl lg:text-4xl font-black text-[#09111E] leading-tight uppercase tracking-tight">
                Welcome Back, <span className="text-[#09111E]">{user?.enterpriseName || 'Enterprise'}!</span>
              </h2>
          ...
          <div className="flex items-center gap-6 relative z-10">
            <div className="flex items-center gap-4">
              <button
                className="p-4 text-[#09111E] hover:bg-brand-100 bg-brand-50 rounded-md border border-brand-100 transition-all hover:shadow-md"
                onClick={() => navigate('/')}
                title="Landing Page"
              >
                <MdHome size={24} />
              ...
              <button
                className={`p-4 rounded-md border transition-all hover:shadow-md ${activeTab === 'profile'
                  ? 'bg-[#09111E] text-white border-[#09111E] shadow-xl'
                  : 'text-[#09111E] hover:bg-brand-100 bg-brand-50 border-brand-100'
                  }`}
            ...
            <div className="relative" ref={notificationRef}>
              <button
                ...
                className="p-4 text-[#09111E] hover:bg-brand-100 bg-brand-50 rounded-md border border-brand-100 transition-all hover:shadow-md relative group/bell"
              >
                ...
                {notifications.length > 0 && (
                  <span className="absolute -top-1 -right-1 bg-[#09111E] text-white text-[10px] font-black rounded-md h-5 px-2 flex items-center justify-center border-2 border-white animate-bounce">
                ...
                <div className="absolute right-0 mt-6 w-96 bg-white border border-brand-100 rounded-md shadow-2xl z-[100] overflow-hidden animate-in fade-in zoom-in duration-300">
                  <div className="p-2" onClick={(e) => e.stopPropagation()}>
                    <div className="px-8 py-6 border-b border-brand-50 flex items-center justify-between bg-brand-50/50">
                      <div className="flex items-center gap-4">
                        <MdNotifications className="text-[#09111E] text-xl" />
                        <h3 className="text-lg font-black text-[#09111E] uppercase tracking-tight">Notifications</h3>
                      </div>
                      <span className="text-[10px] bg-[#09111E] px-4 py-1.5 rounded-md text-white font-black uppercase tracking-widest">{notifications.length} New Records</span>
                    </div>
                          ...
                          >
                            <div className="flex gap-6">
                              ...
                              <div className="flex-1 min-w-0">
                                <p className="text-base font-black text-[#09111E] group-hover:text-[#09111E] transition-colors leading-tight truncate uppercase tracking-tight">{notification.title}</p>
                    ...
                    <div className="p-8 bg-brand-50/30 border-t border-brand-50">
                      <button
                        ...
                        className="w-full py-5 text-xs text-white bg-[#09111E] hover:bg-[#09111E] rounded-md transition-all font-black uppercase tracking-widest shadow-xl active:scale-95"
                      >
                        Access Audit Logs
                      </button>
                    </div>
