...
const EditModal = ({ isOpen, onClose, data, title, onSave, fields }) => {
  ...
  return (
    <div className="fixed inset-0 bg-chocolate-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-6 font-afacad cursor-default animate-in fade-in duration-300">
      <div className="bg-white border border-gray-100 rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden relative flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-10 max-h-[70vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            {fields.map((field) => (
              <div key={field.key} className={`${field.fullWidth ? 'md:col-span-2' : ''} space-y-3`}>
                <label className="block text-sm font-bold text-gray-400 px-1 capitalize">
                ...
                {field.type === 'textarea' ? (
                  <textarea
                    ...
                    rows="4"
                    className="w-full px-8 py-5 bg-white border border-gray-100 rounded-lg text-black placeholder:text-chocolate-200 focus:outline-none focus:ring-4 focus:ring-chocolate-50 transition-all resize-none shadow-sm leading-relaxed"
                  ...
                  <div className="relative group">
                    <select
                      ...
                      onChange={handleChange}
                      required={field.required}
                      className="w-full px-8 py-5 bg-white border border-gray-100 rounded-lg text-black focus:outline-none focus:ring-4 focus:ring-chocolate-50 transition-all appearance-none cursor-pointer shadow-sm relative z-10"
                    >
                    ...
                    <div className="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none z-20 text-gray-300 group-hover:text-black transition-colors">
                  ...
                  <input
                    ...
                    min={field.min}
                    max={field.max}
                    className={`w-full px-8 py-5 bg-white border border-gray-100 rounded-lg text-black placeholder:text-chocolate-200 focus:outline-none focus:ring-4 focus:ring-chocolate-50 transition-all shadow-sm ${field.disabled ? 'bg-gray-50/50 cursor-not-allowed opacity-60' : ''}`}
          ...
          <div className="flex items-center justify-end gap-8 pt-6">
            <button
              type="button"
              onClick={onClose}
              className="px-8 py-5 text-gray-400 hover:text-black transition-all font-bold active:scale-95"
            >
              Cancel Operation
            </button>
            <button
