        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-10 max-h-[70vh] overflow-y-auto custom-scrollbar relative z-10">
              ...
              <div key={field.key} className={`${field.fullWidth ? 'md:col-span-2' : ''} space-y-4`}>
                ...
                {field.type === 'textarea' ? (
                  <textarea
                    ...
                    rows="5"
                    className="w-full px-8 py-6 bg-white/5 border border-white/5 rounded-md text-white font-semibold tracking-tight text-lg placeholder:text-brand-300/20 focus:outline-none focus:ring-4 focus:ring-accent-400/10 focus:border-accent-400/50 transition-all resize-none shadow-inner leading-relaxed"
                  ...
                  <div className="relative group">
                    ...
                    <div className="absolute right-6 top-1/2 -translate-y-1/2 pointer-events-none z-20 text-brand-300 group-hover:text-white transition-colors opacity-40">
                  ...
                  <input
                    ...
                    min={field.min}
                    max={field.max}
                    className={`w-full px-8 py-6 bg-white/5 border border-white/5 rounded-md text-white font-semibold tracking-tight text-lg placeholder:text-brand-300/20 focus:outline-none focus:ring-4 focus:ring-accent-400/10 focus:border-accent-400/50 transition-all shadow-inner ${field.disabled ? 'cursor-not-allowed opacity-30 bg-black/20' : ''}`}
          ...
          <div className="flex items-center justify-end gap-10 pt-12 border-t border-white/5">
            <button
              type="button"
              onClick={onClose}
              className="text-xs font-semibold text-brand-300 tracking-wide hover:text-white transition-all active:scale-95 opacity-60"
            >
              Cancel
            </button>
            <button
