      ...
      <div className="bg-white border border-brand-100 rounded-md shadow-[0_50px_100px_-20px_rgba(9,17,30,0.3)] w-full max-w-2xl overflow-hidden relative flex flex-col">
        <div className="p-12 border-b border-brand-50 flex items-center justify-between bg-brand-50/30">
          <div className="flex items-center gap-6">
            ...
            <div>
              <h2 className="text-4xl font-black text-[#09111E] leading-none">Financial Audit</h2>
              <p className="text-[10px] font-black text-white/60 tracking-normal mt-3 italic">Comprehensive Transaction Log</p>
            </div>
          </div>
          <div className="flex items-center gap-6">
            {!financial.paid && (
        ...
        <div className="p-10 space-y-10 max-h-[70vh] overflow-y-auto custom-scrollbar">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-4">
              <label className="block text-[10px] font-black text-white/60 tracking-normal px-1 italic">Classification</label>
              <div className="px-10 py-8 bg-brand-50/50 border border-brand-100 rounded-md shadow-inner text-center">
                <span className={`inline-flex px-8 py-3 text-[10px] font-black  tracking-normal rounded-md border ${financial.type?.toLowerCase() === 'credit' ? 'bg-emerald-100 text-emerald-800 border-emerald-200' : 'bg-rose-100 text-rose-800 border-rose-200'}`}>
                  {financial.type}
                </span>
              </div>
            </div>
            <div className="space-y-4">
              <label className="block text-[10px] font-black text-white/60 tracking-normal px-1 italic">Settlement Progress</label>
              <div className="px-10 py-8 bg-brand-50/50 border border-brand-100 rounded-md shadow-inner text-center">
                <span className={`inline-flex px-8 py-3 text-[10px] font-black  tracking-normal rounded-md border ${financial.paid ? 'bg-emerald-100 text-emerald-800 border-emerald-200' : 'bg-amber-100 text-amber-800 border-amber-200'}`}>
          ...
          <div className="space-y-4">
            <label className="text-[10px] font-black text-white/60 tracking-normal px-1 italic flex items-center gap-4">
            ...
            <div className="px-12 py-10 bg-[#09111E] rounded-md shadow-2xl relative overflow-hidden group">
              ...
              <div className="relative z-10 flex items-baseline justify-between">
                <span className={`text-6xl font-black  ${financial.type?.toLowerCase() === 'credit' ? 'text-emerald-400' : 'text-rose-400'}`}>
                  {financial.type?.toLowerCase() === 'credit' ? '+' : '-'}{financial.amount.toLocaleString()}
                </span>
                <span className="text-xl text-white/40 font-black tracking-normal">FRW_UNIT</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
            <div className="space-y-4">
              <label className="flex text-[10px] font-black text-white/60 tracking-normal px-1 items-center gap-4 italic">
              ...
              <div className="px-10 py-8 bg-brand-50/50 border border-brand-100 rounded-md shadow-inner">
                <p className="text-[#09111E] font-black text-[10px] leading-relaxed">{financial.description}</p>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="px-10 py-8 bg-brand-50/50 border border-brand-100 rounded-md shadow-inner text-center">
                <p className="text-[#09111E] font-black text-[10px]">{formatDate(financial.deadline)}</p>
              </div>
            </div>
            ...
            {financial.collateral && (
              <div className="md:col-span-2 space-y-4">
                ...
                <div className="px-10 py-8 bg-brand-50/50 border border-brand-100 rounded-md shadow-inner text-center">
                  <p className="text-[#09111E] font-black text-[10px] leading-relaxed">{financial.collateral}</p>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="bg-brand-50/50 border border-brand-100 p-10 rounded-md shadow-inner relative overflow-hidden">
            <h4 className="text-[10px] font-black text-white/60 tracking-normal mb-8 italic">Diagnostic Timestamps</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-12 relative z-10">
              <div>
                <span className="block text-[10px] font-black text-white/70 tracking-normal mb-3">Kernel Initialization</span>
                <p className="text-[10px] text-[#09111E] font-black">{formatDate(financial.createdAt || financial.deadline)}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-12 border-t border-brand-50 flex items-center justify-end bg-brand-50/20">
          <button
            onClick={onClose}
            className="px-12 py-5 bg-[#09111E] text-white rounded-md font-black tracking-normal text-[10px] transition-all hover:bg-[#09111E] active:scale-95 shadow-2xl relative overflow-hidden group/close"
          >
