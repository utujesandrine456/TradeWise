...
export function loggerMiddleware() {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    res.on('finish', () => {
      ...
      // File logging
      const logMessage = `[${new Date().toISOString()}] ${method} ${url} | Status: ${statusCode} | Host: ${host} | Client: ${clientName} | DataLength: ${dataLength} bytes | Duration: ${duration}ms | UserId: ${(req as any).user?.sub || ''}`;
