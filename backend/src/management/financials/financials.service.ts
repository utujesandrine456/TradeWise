...
export class FinancialsService {
  ...
  public async getFinancial(traderId: string, financialId: string) {
    const stock = await this.findStock(traderId);
    ...
    financial = await this.prismaService.mFinancial.findUnique({
    ...
    const financialAny = financial as any;
    if (!financialAny || financialAny.stockId !== stock.id) {
      throw new BadRequestException("Financial not found");
    }
  ...
  public async markAsPaidBack(traderId: string, financialId: string) {
    const stock = await this.findStock(traderId);
    const financial: any = await this.prismaService.mFinancial.update({
